...
session_start(); //start session
	if(isset($_SESSION["admin"]) && $_SESSION["admin"]==true){ //login successfully
		$untop_by=$_SESSION["adminname"]; // echo $deleted_by;
               
                //check privilege
                $check_privilege="select privilege from is_user where username='$untop_by'";
		...
		//执行置顶
                if($current_privilege>-1){ // 已激活帐号
                     if($current_privilege>2){// 符合管理员（版主）权限
                            $topsql="update `forum_topics` set `status`=0 where topic_id=$topic_id";
		            mysql_query($topsql) or die("Fail to top the topic.".mysql_error());
	                    echo "<script>alert('您是版主，取消置顶帖子成功！');window.location='showtopic.php?topic_id=$topic_id';</script>";
                     }else{
