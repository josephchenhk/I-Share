...
<a href=\"#\"><img src=\"images/logotext.gif\" id=\"logotext\" /></a>  <!--将logotext摆进来，其位置在#logotext中已进行定义 -->
   ...
   <div id=\"panel\">
		...
		<input type=\"hidden\" name=\"topic_id\" value=\"$topic_id\">               <!--收集当前topic_id -->
                ...
                <input type=\"hidden\" name=\"reply_post_id\" value=\"$reply_post_id\">     <!--收集回复对象id -->
		...
		if ((!$_POST[topic_id]) || (!$_POST[post_text])) {
			...
			exit;
               ...
               if($current_privilege>-1){ // 已激活帐号 
                     ...
                     $reply_post_owner = $_POST[reply_post_owner];                    
 
                     if($modified_by == $reply_post_owner){ // 是回帖本人
	                     //modify the post//执行修改
		             $modify_post = "update `forum_reply_posts` set `post_text`='$_POST[post_text]',`post_modify_time`=now() where reply_post_id =$reply_post_id";
